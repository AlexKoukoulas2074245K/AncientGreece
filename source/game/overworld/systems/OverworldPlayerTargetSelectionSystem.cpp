...
void OverworldPlayerTargetSelectionSystem::VUpdate(const float, const std::vector<genesis::ecs::EntityId>& entitiesToProcess) const
{
    ...
    auto targetAreaType = areaTypeMasks::NEUTRAL;
    ...
    {
        CalculateMapTarget(world, targetPosition, targetAreaType);
        const auto& playerStatsComponent = world.GetComponent<UnitStatsComponent>(playerEntity);
        isValidTarget = (playerStatsComponent.mNavigableAreaTypes & targetAreaType) != 0;
    }
    
    if (isValidTarget)
    {
        FindPathToTarget(&world, playerEntity, entityToFollow, targetPosition, targetAreaType);
    }
}
...
void OverworldPlayerTargetSelectionSystem::CalculateMapTarget(genesis::ecs::World& world, glm::vec3& targetPosition, AreaTypeMask& targetAreaType) const
{
    ...
    auto rayDirection = genesis::math::ComputeMouseRayDirection(viewMatrix, projectionMatrix, windowComponent.mRenderableWidth, windowComponent.mRenderableHeight);
    ...
    targetPosition = mapIntersectionPoint;
    targetAreaType = RGB_TO_AREA_TYPE_MASK.count(targetPixel) > 0 ? RGB_TO_AREA_TYPE_MASK.at(targetPixel) : areaTypeMasks::NEUTRAL;
}

///-----------------------------------------------------------------------------------------------

void OverworldPlayerTargetSelectionSystem::FindPathToTarget(genesis::ecs::World* world, genesis::ecs::EntityId playerEntity, genesis::ecs::EntityId targetEntity, glm::vec3 targetPosition, AreaTypeMask targetAreaType) const
{
    sPathThread = std::thread([=]()
    {
        auto targetComponent = std::make_unique<OverworldTargetComponent>();
        
        if (targetEntity != genesis::ecs::NULL_ENTITY_ID)
        {
            targetComponent->mOptionalEntityTarget = targetEntity;
        }
        else
        {
            // Find map entity
            ...
            // Get A* path
            targetComponent->mTargetPositionPath = FindPath(playerTransformComponent.mPosition, targetPosition, playerStatsComponent.mNavigableAreaTypes, mapDimensions, navmapTexture);
            targetComponent->mTargetAreaType = targetAreaType;
        }
