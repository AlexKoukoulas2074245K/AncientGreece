...
std::list<genesis::ecs::EntityId> QuadtreeSceneGraph::VGetCollisionCandidates(const genesis::ecs::EntityId referenceEntityId) const
{
    ...
    const auto& collidableDimensions = world.GetComponent<CollidableComponent>(referenceEntityId).mCollidableDimensions;
    
    std::list<genesis::ecs::EntityId> collisionCandidates;
    InternalGetCollisionCandidates(referenceEntityId, entityPosition, collidableDimensions, collisionCandidates);
...
void QuadtreeSceneGraph::VPopulateSceneGraph(const std::vector<genesis::ecs::EntityId>& physicallySimulatedEntities)
{
    ...
    for (const auto& entityId: physicallySimulatedEntities)
    {
        ...
        const auto& collidableDimensions = world.GetComponent<CollidableComponent>(entityId).mCollidableDimensions;
        
        InsertObject(entityId, entityPosition, collidableDimensions);
    }
}
